...
class Entregadores extends BaseController {
    ...
    public function upload(int $id = null){
        ...
        $imagem = $this->request->getFile('foto-entregador');
        ...
        $tipoImagemLimpo = explode('/', $imagem->getMimeType());

        $tiposPermitidos = ['jpeg','png','webp'];

        if (!in_array($tipoImagemLimpo[1],$tiposPermitidos)){

            return redirect()->back()->with('atencao', 'O arquivo não tem o formato permitido. São aceitos apenas: ' . implode(', ', $tiposPermitidos) . '.');
        ...
        // ------------- A partir deste ponto faço o store da imagem --------------
